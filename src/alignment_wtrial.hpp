#ifndef ALIGNMENT_WTRIAL_HPP
#define ALIGNMENT_WTRIAL_HPP

#include "stepwise_colorable.hpp"
#include "alignment_utilities.hpp"

namespace alignment_wtrial{
...
template<typename DataClass> DataClass read() {
	...
	while (AP.nextAlignment()) {
		...
		// ══════════ WTRIAL-DEBUG-MOD BEGIN: weight dump (gated by --dump-chunk-weights, default OFF) ══════════
...
public:
	...
	static std::pair<string, string> programNames() noexcept {
		return { "alignment_wtrial", "Alignment-Weighted Trial: per-alignment sequence-similarity weighting" };
	}
...
protected:
	string introduction() const noexcept override {
		return R"YOHANETYO(# Alignment-Weighted Trial: TRIAL with per-alignment sequence-similarity weighting

Alignment-Weighted Trial is a variant of TRIAL that restricts quartet scoring to only those quartets that contain a reference species (R), and additionally weights each alignment by its average sequence similarity. This is designed for phylogenomic analyses where multiple fasta files are aligned to a common reference genome.
...
4. Per-alignment weights (based on sequence similarity) scale the pairwise score contribution of each alignment.
...
While TRIAL scores all quartets that include the reference species equally across alignments, Alignment-Weighted Trial down-weights alignments with lower sequence similarity. This reduces the impact of poorly aligned regions while retaining the reference-species triangulation anchor.
...
TRIAL uses a `fasta2ref` file to specify the mapping between fasta alignment files and their reference species. Alignment-Weighted Trial accepts the same input format as TRIAL.
...
```
	...
	string programName() const noexcept override { return "alignment_wtrial"; }

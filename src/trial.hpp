...
public:
	...
	static std::pair<string, string> programNames() noexcept {
		return { "trial", "TRIAL: Coalescence-aware Alignment-based Species Tree EstimatoR with TRI-reference" };
	}
...
protected:
	string introduction() const noexcept override {
		return R"YOHANETYO(# TRIAL: Coalescence-aware Alignment-based Species Tree EstimatoR with TRI-reference

TRIAL is a variant of CASTER that restricts quartet scoring to only those quartets that contain a reference species (R). This is designed for phylogenomic analyses where multiple fasta files are aligned to a common reference genome.
...
While CASTER scores all possible quartets across the alignment, TRIAL only scores quartets that include the designated reference species. This makes it suitable for analyses where the reference genome provides a consistent triangulation anchor.
...
TRIAL uses a `fasta2ref` file to specify the mapping between fasta alignment files and their reference species.
...
## Usage
```
bin/trial -i example/fasta2ref.txt -o output.tre
```
)YOHANETYO";
	}

	string programName() const noexcept override { return "trial"; }

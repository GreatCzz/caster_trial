#ifndef ALIGNMENT_WCASTER_HPP
#define ALIGNMENT_WCASTER_HPP

#include "stepwise_colorable.hpp"
#include "alignment_utilities.hpp"

namespace alignment_wcaster{
...
ChangeLog logColor("Color",
	...
	"2026-09-04", "Zuizhi Chen", "WCASTER: global quartet scoring with per-alignment weighting (colorWeight/colorPairWeight)", "minor");
...
ChangeLog logDriverHelper("DriverHelper",
	...
	"2026-09-04", "Zuizhi Chen", "WCASTER: per-alignment weight computation in read()", "minor");
...
// ══════════ WCASTER-MOD ⑨ BEGIN: fasta2ref input + per-alignment weight computation (照 wtrial) ══════════
template<typename DataClass> DataClass read() {
	...
	string tempListFile = "/tmp/alignment_wcaster_fasta_list_" + std::to_string(std::hash<std::thread::id>()(std::this_thread::get_id())) + ".txt";
	{
	...
	while (AP.nextAlignment()) {
		...
		// WCASTER: per-alignment weights from Hamming distance to ref (all chunks share the same weights)
		...
		while (AP2.nextSeq()) {
			...
			// Weight: Hamming distance to ref over all non-gap sites of the whole alignment
			cnt_t w = 1.0;
		...
		// WCASTER: assign weights to all elements of this alignment
...
ChangeLog logDriver("Driver",
	...
	"2026-09-04", "Zuizhi Chen", "WCASTER: add alignment_wcaster driver", "minor");
...
public:
	...
	static std::pair<string, string> programNames() noexcept {
		return { "alignment_wcaster", "Alignment-Weighted CASTER: CASTER with per-alignment sequence-similarity weighting" };
	}
...
```
	...
	string programName() const noexcept override { return "alignment_wcaster"; }

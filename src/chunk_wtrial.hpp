#ifndef CHUNK_WTRIAL_HPP
#define CHUNK_WTRIAL_HPP

#include "stepwise_colorable.hpp"
#include "alignment_utilities.hpp"

namespace chunk_wtrial{
...
template<typename DataClass> DataClass read() {
	...
	while (AP.nextAlignment()) {
		...
		for (size_t i = 0; i < nChunk; i++) {
			...
			element.speciesWeights.resize(taxon2row.size(), 1.0); // WTRIAL: init all weights to 1.0
...
public:
	...
	static std::pair<string, string> programNames() noexcept {
		return { "chunk_wtrial", "Chunk-Weighted Trial: per-element sequence-similarity weighting" };
	}
...
protected:
	string introduction() const noexcept override {
		return R"YOHANETYO(# Chunk-Weighted Trial: TRIAL with per-chunk sequence-similarity weighting

Chunk-Weighted Trial is a variant of TRIAL that restricts quartet scoring to only those quartets that contain a reference species (R), and additionally weights each alignment chunk (element) by its local sequence similarity. This is designed for phylogenomic analyses where multiple fasta files are aligned to a common reference genome.
...
4. Each alignment is split into chunks (elements); per-chunk weights based on local sequence similarity scale the pairwise score contribution of each chunk.
5. The reference species is always placed first during heuristic search to avoid information loss.

## Key difference from TRIAL
While TRIAL scores all quartets that include the reference species equally across the alignment, Chunk-Weighted Trial down-weights poorly conserved chunks at a finer per-chunk resolution. This retains the reference-species triangulation anchor while mitigating the effect of local misalignment.
...
TRIAL uses a `fasta2ref` file to specify the mapping between fasta alignment files and their reference species. Chunk-Weighted Trial accepts the same input format as TRIAL.
...
```
	...
	string programName() const noexcept override { return "chunk_wtrial"; }
